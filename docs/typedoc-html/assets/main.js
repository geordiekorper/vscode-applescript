	...
	var ye = et((me, ge) => {
		(function () {
			...
			(t.tokenizer = (e, n) => {
				...
				if (Array.isArray(e)) return e.map((f) => new t.Token(t.utils.asString(f).toLowerCase(), t.utils.clone(n)));
...
`),
	...
	function at(t, e, n, r) {
		...
		if (s.length === 0 && i) {
			const a = window.translations.search_no_results_found_for_0.replace('{0}', ` "<strong>${te(i)}</strong>" `);
			Pe(n, a);
			return;
		}
